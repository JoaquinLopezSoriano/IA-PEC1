...
namespace Script.Citizen.State.Destination
{
    ...
    public class FindDestinationState : ICitizenDestinationState
    {
        ...
        public FindDestinationState (FsmCitizen fsmCitizen)
        {
            _fsm = fsmCitizen;
            _areaMask = NavMesh.GetAreaFromName(Constant.NavMesh.CITIZEN_WALK)+1; // plus 1 so the resulted value is 1
        }
        ...
        private bool RandomPoint(Vector3 center, float range, out Vector3 result)
        {
            ...
            if (NavMesh.SamplePosition(new Vector3(randomPoint.x, 0.5f, randomPoint.z), out hit, 1.0f, _areaMask))
            {
